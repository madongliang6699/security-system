...
quit或者exit只是退出了当前的arthas客户端连接，没有关闭arthas的服务器或会话。

stop是关闭了arthas的服务器，所有的arthas的客户端session会话会断掉。快捷键是 ctr + z

一般还是使用ctr + z 直接退出服务吧,因为quit退出后不知道怎么重新连接上,重新java -jar启动服务又提示端口被占用.