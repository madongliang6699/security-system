...
drwxr-xr-x. 2 root root  32 4月   5 13:37 jvm启动参数.md
...
sudo -i  ：进入一个持续的root环境，以root权限执行命令，但并不是切换到root用户。
sudo -s  ：也是进入一个持续的root环境，以root权限执行命令，和sudo  -i的区别是保存了原来普通用户的环境，比如pwd命令出来的是当前普通用户的主目录。
