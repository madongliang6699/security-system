...
mysql是如何来识别一个用户的呢：
mysql为了安全性考虑，采用主机名+用户名来判断一个用户的身份，因为在互联网中很难通过用户名来判断一个用户的身份，
但是我们可以通过ip或者主机名判断一台机器，某个用户通过这个机器过来的，我们可以识别为一个用户，所以mysql中采用用户名+主机名来识别用户的身份。
当一个用户对mysql发送指令的时候，mysql就是通过用户名和来源（主机）来断定用户的权限。
...
权限生效时间：
用户及权限信息放在库名为mysql的库中，mysql启动时，这些内容被读进内存并且从“此时”生效，所以如果通过直接操作这些表来修改用户及权限信息的，需要 重启mysql 或者 执行 flush privileges; 才可以生效。
用户登录之后，mysql会和当前用户之间创建一个连接，此时用户相关的权限信息都保存在这个连接中，存放在内存中，此时如果有“其他地方”修改了“当前用户”的权限，这些变更的权限会在下一次登录时才会生效。
【也就是说：当前已经登录的用户，即便权限被改成无权限登录了，也不会立即被切断连接，下次再登录时才生效。】
...
密码可以省略，表示无密码登录.
例如:create user user_mdl identified by '123987456Mdl';
...
注意：
通过表的方式修改之后，需要执行flush privileges;才能对用户生效。【原因上面有讲。这也说明前两种方式是直接生效的，但是应该是该用户下次登录时才生效，如果已经正在登录了，就暂时不生效，原因上面有讲】
...
【疑问：这里指定的主机IP和创建用户的时候指定的IP是什么关系，以哪个为准，如果两个地方指定了不同的IP，怎么起作用的】

WITH GRANT OPTION： 这个选项表示该用户可以将自己拥有的权限授权给别人(用法在文档末尾)

备注：可以使用GRANT重复给用户添加权限，权限叠加，比如你先给用户添加一个select权限，然后又给用户添加一个insert权限，那么该用户就同时拥有了select和insert权限。

示例：
grant all on *.* to 'test1'@'%';
说明：给test1授权可以操作所有库所有权限, 所有权限中也包括可以授予其他用户权限的权限（），相当于dba
...
USAGE 这个权限的意思是只能登录,不能有其他操作.是创建用户后默认必须有的权限.
...
~~~text
WITH GRANT OPTION 是 MySQL 中用于授予权限的一种特殊选项，它允许一个用户在获得某些权限后，还可以将这些权限授予其他用户。

基本用法
当你授予某个用户权限时，如果同时使用了 WITH GRANT OPTION，这个用户就不仅仅能够执行被授予的操作，还可以将这些操作权限授予其他用户。

语法:
GRANT privilege_list ON database_name.table_name TO 'user'@'host' IDENTIFIED BY 'password' WITH GRANT OPTION;

示例  
假设你有一个用户 alice，你希望授予她对数据库 mydb 的所有表的所有权限，并且允许她将这些权限授予其他用户，你可以使用以下命令：  
GRANT ALL PRIVILEGES ON mydb.* TO 'alice'@'localhost' IDENTIFIED BY 'password' WITH GRANT OPTION;
在这个例子中：  
ALL PRIVILEGES 授予了 alice 对 mydb 数据库下所有表的所有权限。
WITH GRANT OPTION 允许 alice 将她所拥有的权限授予其他用户。

WITH GRANT OPTION 的作用
传递权限：被授予 WITH GRANT OPTION 的用户，可以将他/她拥有的权限再传递给其他用户。
这对于管理权限层次结构非常有用。

层层递进：如果 alice 使用 WITH GRANT OPTION 将权限授予 bob，并且 bob 也使用 WITH GRANT OPTION 将权限授予 charlie，那么 charlie 也将能够再继续授予其他用户这些权限。

注意事项
安全性问题：由于 WITH GRANT OPTION 允许用户授予权限，这可能导致不受控制的权限扩散，因此在授予该选项时需要非常谨慎。

循环依赖：MySQL 不允许用户循环地相互授予权限。例如，如果 alice 授予 bob 权限，并且 bob 尝试将权限授予 alice，这将被视为循环依赖并被拒绝。

撤销权限：如果要撤销 WITH GRANT OPTION 授予的权限，可以使用 REVOKE 命令：
REVOKE ALL PRIVILEGES, GRANT OPTION FROM 'alice'@'localhost';
这条命令不仅会撤销 alice 的所有权限，还会撤销她传递给其他用户的权限。

实际场景
在开发和生产环境中，WITH GRANT OPTION 通常用于数据库管理员（DBA）需要让某个用户管理特定权限时。
例如，DBA 可以授予一个开发负责人管理自己项目数据库的权限，并允许他将部分权限授予其他开发人员。

总结
WITH GRANT OPTION 是 MySQL 中一个强大的选项，用于控制权限的分配和传递。它允许用户不仅执行权限授予的操作，还可以将这些权限授予其他用户。
然而，由于其潜在的安全风险，使用时需要小心，确保只有可信任的用户才拥有此选项。
