...
说明，通过下面很多知识点的综合决定, 使用docker swarm一般必须向创建一个overlay驱动类型的docker网络,
这个类型的网络可以实现容器的跨节点访问,也就是: swarm使用overlay驱动类型的网络, 可以保证不同节点的容器在一个网络里.
可以实现容器的跨节点访问,并且,有服务自动发现的功能, 当你在 Swarm 中启动多个容器时，Docker 网络会为它们提供一个内置的DNS解析服务。
容器可以通过服务名称（而不是 IP 地址）进行相互访问。这种自动的服务发现使得容器间的通信更加动态和灵活，
避免了手动配置 IP 地址。
...
创建swarm集群的时候指定该网络:
docker service create --name my-nginx -p 80:80 --replicas 3 --network my-overlay-net 镜像名称
